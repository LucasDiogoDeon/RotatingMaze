...
import static com.gmail.lucasddeon.rotatingmaze.helpers.ScreenHelper.*;

/**
 *
 */
public class GameDrunkModeRenderer extends GameRenderer {
    ...
    protected void showInitialScreen() {
        ...
        final GameDrunkModeRenderer instance = this;
    ...
    public GameDrunkModeRenderer(GameScreen fromScreen, GameWorld gameWorld, GameOptions gameOptions) {

        super(GameMode.DRUNK);
    ...
    protected void verifyAccelerometer() {
        ...
        final float slider_multiplier = 2;
        float accY = (Gdx.input.getAccelerometerY() * multiplier * slider_multiplier);

        if (accY < min) accY = min;
        else if (accY > max) accY = max;

        slider.setValue(accY);
    ...
    protected void initGameObjects() {
        ...
        slider = new Slider(-90, 90, 1, true, AssetLoader.skin, "knob");
